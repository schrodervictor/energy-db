...
EnergyTable.prototype.query = function query(doc, callback) {
  ...
  var readQuery = energyQuery.getQuery();

  if (this.queryHasAnIndexHashKey(doc)) {
    readQuery['IndexName'] = this.findSuitableIndex(doc);
  }
...
EnergyTable.prototype.delete = function _delete(doc, callback) {
  ...
  var deleteQuery = energyQuery.getQuery();
...
EnergyTable.prototype.update = function update(doc, updates, callback) {
  ...
  var updateQuery = energyQuery.getUpdateQuery(updates);

  this.db.getConnector().updateItem(updateQuery, function postQuery(err, data) {
    if (err) return callback(err);
