...
var EnergyQueryFactory = require('./energy-query').EnergyQueryFactory;
...
EnergyTable.prototype.putItem = function putItem(item, options, callback) {
  ...
  this.queryFactory.getInsertQuery(item, function(err, query) {
    if (err) return callback(err);
...
EnergyTable.prototype.query = function query(doc, options, callback) {
  ...
  if (this.hashKey in doc || this.queryHasAnIndexHashKey(doc)) {
    method = 'query';
    this.queryFactory.getQuery(doc, sendQuery);
  } else {
    method = 'scan';
    this.queryFactory.getScanQuery(doc, sendQuery);
  }
...
EnergyTable.prototype.delete = function _delete(doc, options, callback) {
  ...
  this.queryFactory.getDeleteQuery(doc, function(err, query) {
    if (err) return callback(err);
...
EnergyTable.prototype.update = function update(doc, updates, options, callback) {
  ...
  if (isUpdate) {
    method = 'updateItem';
    this.queryFactory.getUpdateQuery(doc, updates, sendQuery);
  } else {
    method = 'putItem';
    this.queryFactory.getReplaceQuery(doc, updates, sendQuery);
  }
